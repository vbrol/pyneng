...
def write_last_log_to_csv(source_log, output):
    ...
    with open(source_log) as f:
        ...
        for row in reader:
            name,email,date=row
            if not email in result or convert_str_to_datetime(date)>convert_str_to_datetime(result[email][1]):
                result[email]=[name,date]
        for email,other in result.items():
            reslist.append([other[0],email,other[1]])
        print(reslist)
    with open(output,'w') as fout:
        writer = csv.writer(fout)
        writer.writerows(reslist)

write_last_log_to_csv('mail_log.csv','174.txt')
